...
// @since 2022-09-21T15:06:06.803Z (UTC)
...
let pageTitle = "CHANNEL ID 기준 PLAYLIST 목록 정보 조회";
let headers = ["id", "title"]; // MODIFY_HERE
...
router.get("/", function (req, res, next) {
  ...
  // ADDITIONAL PARAM
  const channelId = req.query.channelId || ""; // MODIFY_HERE
  ...
  if (channelId) {
    ...
    youtube.getPlaylists(channelId).then((response) => {
      let rows = response.map((x) => {
        return { id: x.id, title: x.snippet.title };
      });
